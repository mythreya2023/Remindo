...
public function storecustmr($strdls,$hspn,$norl){
    ...
    $vfdpndusr=$this->chckifpnd($this->enc($strdls['stnmr'],$this->strec,'mtr'));
        ...
        <div class='csmruiofstoreenvicntrdvbx'>
            <div class='strebsnsprfdtlscntrdvbox'>
                <div class='stronrpgtphdr'>
                    ...
                    <div class='strinfottldvbx'>Store Info</div>";
                    ...
                    if(!$hspn){$strcmr.="<div class='remindo-pin-button pinstrtbtn' style='height:fit-content;padding: 3px 6px;
                        position: relative;
                        right: -17%;
    ...
    if($hspn){$strcmr.="<div class='popupbackground streppupbx'>
        <div class='popupcontainerbox streppupbx'>
            ...
            if($this->dec($strdls['strbupimg'],$this->strec)!=""&&file_exists("../qrstsbprfpcs/".$this->dec($strdls['strbupimg'],$this->strec))){$strcmr.="
                <div class='upimgdsplyppupbx'><h4 style='margin:2px;'>Scan Qr to pay</h4><img src='../qrstsbprfpcs/".$this->dec($strdls['strbupimg'],$this->strec)."' style='border-radius:5px;width:200px;'></div><hr>";}
            $strcmr.= "
            ...
            <p style='color:gray;text-align:center;font-size:13px;'><strong>Note:</strong>Pay the payment to the above mobile number with the above available UPI methods. Enter your payment method profile name and payment method name and click the 'Verify payment' button below to ask the store about your payment. Eg: Ravi via Paytm. Or you can call to the above mobile number and pay.";if($this->dec($vfdpndusr['strcstmrcsonavbl'],$this->strec)!=1||$this->dec($strdls['stravblcstoevyon'],$this->strec)==1){$strcmr.="";}$strcmr.="</p>
    ...
    OneSignal.push(function () {
      OneSignal.init({
      appId: "1b902a25-1540-49ee-a751-f263ec8c74fb",
    ...
    <div class='push-img'>
      <img
        src='https://remindo.in/includes/fn_img/alaram96.png'
    ...
    <script src='https://remindo.in/stores/push.js'></script>
    <script src='https://remindo.in/js/comfle.js'></script>
    <script src='https://remindo.in/stores/srcmspwress.js'></script>
    <script src='https://remindo.in/js/cmnlbxs.js'></script>
    <script src='https://remindo.in/stores/fhdtacse.js'></script>
    <script src='https://remindo.in/js/cobnfns.js'></script>
    <script async src='https://www.googletagmanager.com/gtag/js?id=G-48RXD6N0M6'></script>
    <script>
      window.dataLayer = window.dataLayer || [];
      function gtag(){dataLayer.push(arguments);}
      gtag('js', new Date());gtag('config', 'G-48RXD6N0M6');
    </script>
    ";
    $strcmr.="
...
protected function prdtlrgdsply($isstr,$str){
    ...
    <div class='cmn-lbx-pdt-dsplyr-cntngdvbx'>
        ...
        <div class='lbx-pdt-adrmqty-mrgn lbx-qtyctngdvbx'>
        ...
        <div class='lbx-pdt-adqty-dsply-bx'>
            <div class='lbxpthlfcmncls pdtadtolstbtn lbx-pdtshr-btn shrpstbn' data-ttl='Remindo.in' data-txt='' data-pic='' data-url='https://remindo.in/' style='cursor:pointer;'><i class='fas fa-share remindosymbols' style='font-weight:500;'></i>Share</div>
...
public function vfyusr($usr){
    ...
    if($s!=0){
        if($this->dec($s['strbsnonr'],$this->iky)==htmlentities($_SESSION['ssndi'])){return "o";}else{
            if($this->vfycmrol($s["stnmr"])!=0){return [$this->dec($this->vfycmrol($s["stnmr"])['strolofpsn'],$this->iky)=="Embedder"?"ebd":"ot",$this->dec($this->vfycmrol($s["stnmr"])['strolofpsn'],$this->iky),$this->chckifpnd($this->enc($s['stnmr'],$this->strec,'strix'))!=0?"c1":"c0"];}
...
elseif($prsclss->vfyusr($str)[0]=="ot"){$s= $prsclss->storewnrs($prsclss->strdtls($str),$prsclss->vfyusr($str)[1]);
    bdyjsn("Store Owner",$s);}
elseif($prsclss->vfyusr($str)[0]=="ebd"){$s= $prsclss->storecustmr($prsclss->strdtls($str),$prsclss->vfyusr($str)[3],$prsclss->vfyusr($str)[1]);
    bdyjsn("Store",$s);}
...
elseif(isset($_POST['ppsttevt'])){if($_POST['ppsttevt']=="frdppstt"&&isset($_GET['s'])){
...
elseif($prsclss->vfyusr($str)[0]=="ebd"){echo $prsclss->storecustmr($prsclss->strdtls($str),$prsclss->vfyusr($str)[3],$prsclss->vfyusr($str)[1]);}
